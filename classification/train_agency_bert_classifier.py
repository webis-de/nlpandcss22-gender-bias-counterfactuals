...
def balance_agency(row, col):   
    '''balances the agency training examples'''
    
    if row.agency == 'agency_pos': 
        row[col] = random.Random(42).sample(row[col], 28)
    return row

pa = pa.progress_apply(lambda row: balance_agency(row, 'sents_masked'), axis=1)
